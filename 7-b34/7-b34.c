...
int main()
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		scanf("%d %s %c %f %s", &new_student.num, &new_student.name, &new_student.sex, &new_student.score, &new_student.addr);

		printf("请输入要插入元素的位序[1..%d]:", ListLength(list)+1);
		scanf("%d", &pos);

		if (ListInsert_DuL(&list, pos, new_student))
		{
			printf("在%d位置插入元素成功\n新表为:\n",pos);
			PrintList(list);
			printf("\n第%d次演示结束\n",i+1);
